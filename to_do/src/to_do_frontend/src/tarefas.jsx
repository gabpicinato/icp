...
function tarefas() {
  ...
  useEffect(() => {
    carregarTarefasETotais();
  }, []);

  async function carregarTarefasETotais() {
  ...
  async function atualizarTotais() {
    const emAndamento = await to_do_backend.totalTarefasEmAndamento();
    const concluidas = await to_do_backend.totalTarefasConcluidas();
    setTotalEmAndamento(Number(emAndamento));
    setTotalConcluidas(Number(concluidas));
  }
    ...
    <main class="mt-[30px] mx-[30px]">
      <form id="formTarefas" class="flex space-x-4" onSubmit={handleSubmit}>
        <div class="w-[15%]">
          <select
            id="categoria"
            class="block w-full px-4 py-4 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
          >
                  ...
                  <div class="flex items-center">
                    <div class="shrink-0">
                      ...
                      >
                        ...
                        {ta.urgente && (
                          <svg
                            class="w-6 h-6 ms-2 text-yellow-300"
                            aria-hidden="true"
                            xmlns="http://www.w3.org/2000/svg"
                            fill="currentColor"
                            viewBox="0 0 22 20"
